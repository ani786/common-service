...
public class AuthTokenValidator implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
        ...
        if (!requestUri.contains(VERSION2) || enabled == null || !TRUE.equalsIgnoreCase(enabled)) {
    ...
    private boolean isAllowed(final Map<String, Object> permissionMap, final HttpServletRequest httpServletRequest,
        ...
        Optional.ofNullable(endpoints)
                .ifPresent((endpoint) -> log
                        .info("allowed endpoints {} and request url: {}", Arrays.toString(endpoint.toArray()),
                                url));

        if (endpoints.contains(url) || endpoints.contains(url + "/")) {
    ...
    public boolean isAllowedFromContextAndServletPaths(final List<Map<String, Object>> permissionList,
        ...
        log.info("inside isAllowedFromContextAndServletPaths contextPath: {} servletPath: {}", contextPath,
                servletPath);
            ...
            if (contextPath.equals(endpoint)) {
                ...
                for (Map<String, Object> metaData : metaDatas) {
                    ...
                    if (action.equals(method)) {
                        ...
                        int noOfAttributes = Integer.parseInt(attributes);
                        if (elements.length >= noOfAttributes) {
                            StringBuilder actualPathBuilder = new StringBuilder();
                            IntStream.range(0, elements.length - noOfAttributes)
                                    .forEach(index -> actualPathBuilder.append("/").append(elements[index]));
                            log.info("servletName: {}, actualServletName: {}", servletName, actualPathBuilder.toString());
                            if (servletName.equals(actualPathBuilder.toString())) {
    ...
    public boolean isAllowedFromEndPointWithRequestUri(final List<Map<String, Object>> permissionList,
        ...
        for (Map<String, Object> map : permissionList) {
            String endpoint = (String) map.get(Constants.END_POINT);
            if (url.equals(endpoint) || endpoint.equalsIgnoreCase(url + "/")) {
                List<Map<String, Object>> metaDatas =
                        (List<Map<String, Object>>) map.get(Constants.META_DATA);
                for (Map<String, Object> metaData : metaDatas) {
                    String action = (String) metaData.get(Constants.ACTION);
